...
const Diagnostics: { [key: string]: DiagnosticMessage } = new Proxy(
  {},
  {
    get(_, message): DiagnosticMessage {
      return {
...
const Debug = {
  ...
  assert(condition: boolean, message?: string) {
    if (!condition) {
      console.error("assertion failed I guess")
      if (message) {
...
function createMapFromTemplate<T>(template: Record<string, T>): Map<T> {
...
/* @internal */ function isUnicodeIdentifierStart(
  code: number,
...
export function createScanner(
  ...
  setText(text, start as number, length as number)
    ...
    while (true) {
      ...
      if (ch === CharacterCodes._) {
        ...
        if (allowSeparator) {
          ...
          error(
            { message: "numeric separators are not allowed here" } as any,
            pos,
            1,
          )
        }
  ...
  function scanNumber(): string {
    ...
    if (
      ...
      if (!finalFragment) {
        error({ message: "digit expected" } as any)
      } else {
  ...
  function scanExtendedUnicodeEscape(): string {
    const escapedValue = scanMinimumNumberOfHexDigits(
      1,
      /*canHaveSeparators*/ false,
    )
  ...
  function getIdentifierToken(): SyntaxKind {
    ...
    if (len >= 2 && len <= 11) {
      ...
      if (ch >= CharacterCodes.a && ch <= CharacterCodes.z) {
        token = textToToken.get(tokenValue) as SyntaxKind
        if (token !== undefined) {
          return token
        }
      }
      ...
      switch (ch) {
        ...
        case CharacterCodes._0:
          if (
            ...
            pos += 2
            let value = scanMinimumNumberOfHexDigits(
              1,
